(function() {
	...
	Simple.prototype.merge = function(another) {
		var len = another.length, i = Number(this.length) || 0, j;
		for( j = 0; j < len; j++ ) {
			this[i++] = another[j];
		}
		this.length = i;
		...
		this.forEach(function(node) {
			let _fn = function(ev) {
				if( selector ) {
					...
					var el = ev.target, match = Array.from(node.querySelectorAll(selector));
					do {
	...
	Simple.prototype.off = function(event, fn) {
		...
		if( event ) {
			this.forEach(function(node) {
				let uid = get_uid(node), fn_list = DATA_PRIVATE[uid]['ev_fn_map'][event];
		...
		// .off()
		this.forEach(function(node) {
			let uid = get_uid(node), fn_map = DATA_PRIVATE[uid]['ev_fn_map'];
			for( event in fn_map) {
